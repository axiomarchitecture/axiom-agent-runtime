...
export class RealJevAdapter implements JevAdapter {
  ...
  async decide(_input: string): Promise<JevDecision> {
    if (!this.config.baseUrl || !this.config.apiKey) {
      throw new Error(
        "RealJevAdapter is not configured. Set JEV_BASE_URL and JEV_API_KEY after confirming Jev's API contract."
      );
    }

    throw new Error(
      "RealJevAdapter is intentionally not guessing Jev's API. Add the documented request/response mapping here."
    );
  }
}
